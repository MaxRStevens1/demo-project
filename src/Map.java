...
public class Map {
	...
	public void setUpMap() {
		...
		// populatelevel has failed to generate the correct number of up / down stairs
		// and now the map will be remade
		int i = populateLevel();
	 ...
	 * offset
	 * 
	 * @return returns the added value of numUpstairs and numDownStairs, with an
	 *         error condition if the added values are NOT 0
	 */
	public int populateLevel() {
		...
		while (doorList.size() != 0) {
			...
			// bit strange math, but just takes random, multiples it by a constant, and
			// multiplies it by 1 or -1
			...
			if (roomPlacementIters >= GameConstants.NUMBER_OF_ROOM_PLACEMENT_ATTEMPTS) {
				pair = null;
				// check done to prevent map generation failing to generate proper number of
				// rooms,
			...
			if (pair != null) {
				...
				if (i < 50 || (room.roomName.contains("StairRoom"))) {
					...
					// set of error conditions, if numUpstairs/downstairs are not 0, it means it
					// will fail at floor connection in dungeon
	...
	public boolean validRoomPlacement(PointPair p, int roomSize) {
		if (p.x2 >= x - 1 || p.y2 >= y - 1 || p.x1 <= 1 || p.y1 <= 1) {
			return false;
		}
			...
			for (int startX = 0; startX < room.xLength; startX++) {
				...
				if (mapTile != null && !mapTile.canWalk) {
					...
					if (roomTile.monsterSpawnFlag) {
						Enemy enemy = new Enemy(roomTile.x, roomTile.y, roomTile);
						roomTile.enemy = enemy;
						enemyList.add(enemy);
					}
	...
	public Room roomSelector() {
		...
		String roomName = GameConstants.ROOM_NAMES[roomSelectedIndex][0]; // the first position of the room_name array
																			// is always the name
	 ...
	 * 
	 * 
	 * @param startPoint start point for pathfinding
	 * @param endPoint   endpoint for pathfinding
	 */
	public Stack<Tile> enemyPathfind(Tile startPoint, Tile endPoint) {
		ArrayList<Tile> openTiles = new ArrayList<Tile>();
		ArrayList<Tile> closedTiles = new ArrayList<Tile>();
		openTiles.add(startPoint);
		while (openTiles.size() > 0) {
			Tile openTile = openTiles.get(0);
			int openTileIndex = 0;
			int currentOpenFVal = movementDistance(openTile, startPoint) + movementDistance(endPoint, openTile);
			for (int i = 0; i < openTiles.size(); i++) {
				// g = point slope calc for startPoint with current tile, y2-y1 / x2-x1
				// h is the same
				// this is the current openTile value
				int tempOpenFVal = movementDistance(openTiles.get(i), startPoint)
						+ movementDistance(endPoint, openTiles.get(i));
				// current lowest ftile value
				if (tempOpenFVal < currentOpenFVal) {
					openTile = openTiles.get(i);
					openTileIndex = i;
			...
			// exit condition
			if (openTile == endPoint) {
				return enemyPathMakerHelper(openTile);
			}

			// gets a single tile rook movement of tiles around itself
			Tile[] adjacentTiles = adjacentEnemyPathTiles(openTile);// childrenRoomConnectorHelper(openTile);
			for (int i = 0; i < adjacentTiles.length; i++) {
				Tile adjacentTile = adjacentTiles[i];
				if (adjacentTile != null && !closedTiles.contains(adjacentTile) && adjacentTile.canWalk) {

					int newCostToAdjacentFVal = movementDistance(adjacentTile, startPoint)
							+ movementDistance(endPoint, adjacentTile);
					if (newCostToAdjacentFVal <= currentOpenFVal && !openTiles.contains(adjacentTile)) {
						adjacentTile.parent = openTile;
						openTiles.add(adjacentTile);
					}
					// exit condition
				} else if (adjacentTile == endPoint) {
					adjacentTile.parent = openTile;
					openTiles.add(adjacentTile);
					return roomPathMakerHelper(endPoint); // creates an stack of tiles to easily parse through
				}
			}
		}
		// failure condition
	...
	/**
	 * 0 1 2 3 X 4 5 6 7 gets adjacent tiles of input tile
	 * 
	 * @param openTile tile to get adjacent tiles too
	 * @return array of adjacent tiles
	 */
	private Tile[] adjacentEnemyPathTiles(Tile openTile) {
		Tile[] adjacent = new Tile[8];

		if (openTile.x != 0 && openTile.y != 0) {
			adjacent[0] = getTile(openTile.x - 1, openTile.y - 1);
		}
		if (openTile.y != 0) {
			adjacent[1] = getTile(openTile.x, openTile.y - 1);
		}
		if (openTile.x + 1 != x && openTile.y != 0) {
			adjacent[2] = getTile(openTile.x + 1, openTile.y - 1);
		}
		if (openTile.x != 0) {
			adjacent[3] = getTile(openTile.x - 1, openTile.y);
		}
		if (openTile.x + 1 != x) {
			adjacent[4] = getTile(openTile.x + 1, openTile.y);
		}
		if (openTile.x != 0 && openTile.y + 1 != y) {
			adjacent[5] = getTile(openTile.x - 1, openTile.y + 1);
		}
		if (openTile.y + 1 != y) {
			adjacent[6] = getTile(openTile.x, openTile.y + 1);
		}
		if (openTile.x + 1 != x && openTile.y + 1 != y) {
			adjacent[7] = getTile(openTile.x + 1, openTile.y + 1);
		}
	...
	/**
	 * Builds a path, ending at endTile and starting at last parent
	 * 
	 * @param endTile input tile
	 * @return stack of tiles, starting with last parent in chain of parents ending
	 *         at endTile
	 */
	private Stack<Tile> enemyPathMakerHelper(Tile endTile) {
		Stack<Tile> path = new Stack<Tile>();
	...
	public Tile getTile(int x, int y) {
		if (x + 1 >= this.x || y + 1 >= this.y || x < 0 || y < 0) {
			return null;
		}
		return map[x][y];
	}
	...
	/**
	 * Gets tile at point p
	 * 
	 * @param p input point
	 * @return tile
	 */
	public Tile getTile(Point p) {
		if (p.x + 1 >= this.x || p.y + 1 >= this.y || p.x < 0 || p.y < 0) {
			return null;
		}
		return map[p.x][p.y];
	}
	...
	/**
	 * removes enemy e, returning true if removed
	 * 
	 * @param e
	 * @return bool
	 */
	...
	/**
	 * Gets enemy at index i in enemyList
	 * 
	 * @param i index
	 * @return enemy
	 */
