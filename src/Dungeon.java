...
public class Dungeon {
	...
	/**
	 * repeats map creation until first map is generated, if iterations are past
	 * constant value exit game, else continue
	 * 
	 * @param x      size of maps
	 * @param y      size of maps
	 * @param player charecter
	 */
	public Dungeon(int x, int y, Player player) {
		...
		while (map.hasMapGenFailed) {
			...
			if (i >= 50) {
				// System.out.println("Error has occured in Map generation: " + i + " attempts
				// failed");
				System.exit(0);
			}
	...
	/**
	 * moves the player up a map, partners are set in moveplayerdown, and the player
	 * should never be able to move up before moving down
	 * 
	 * @return
	 */
	...
	/**
	 * Moves the player up a map, creates a new map, and moves the player into the
	 * partner tile of their stair, set by connectstairs
	 * 
	...
	public boolean movePlayerDown() {
		...
		if (currentDungeonMapIndex + 1 != dungeon.size()) {
			DownStairTile oldPlayerTile = (DownStairTile) dungeon.get(currentDungeonMapIndex).getTile(player.x,
					player.y);
			currentDungeonMapIndex++;
		...
		// then its false as it is not the first floor, and then a bool is sent in, and
		...
		// gets the previous players tile position, adds the new map to the dungeon, and
		// iterates the dungeon map index
		...
		if (connectStairsBetweenNewFloorAndOld()) {
			// another vomit, but gets the partner of the stairtile in the new map, which is
			// set through connectStairs method
	 ...
	 * 
	 * @return T/F on whether or not both sets of stair tiles have had their
	 *         partners set
	 */
	...
	// Getters and setters
	public ArrayList<Map> getDungeon() {
		return dungeon;
	}

	public Map getMap() {
		return dungeon.get(currentDungeonMapIndex);
	}
}
